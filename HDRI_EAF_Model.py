...
def establish_save_output_dict():
    ...
    save_outputs_dict['Raw Iron Ore Input'] = list()
    save_outputs_dict['Metallic Stream into EAF'] = list()
    save_outputs_dict['EAF Output'] = list()
    save_outputs_dict['Actual EAF Output'] = list()
    save_outputs_dict['Plant Life'] = list()
    save_outputs_dict['Lang Factor'] = list()
    save_outputs_dict['H2 Produced per Year'] = list()
    save_outputs_dict['Operating Percent'] = list()
...
def HDRI_EAF_Model():
    ...
    def electrolyzer_npv(tax_rate,interest_rate,electricity_cost,iron_ore_cost,emission_cost,carbon_steel_price,O2_price,el_spec,h2_investment_2020,emission_factor):
        ...
        indirect_emissions_eaf=(el_eaf*emission_factor)/1000
        indirect_emissions_el=(el_elec*emission_factor)/1000
        ...
        #million USD
        total_capital_cost=((electrolyer_cost)+eaf_total_cost+dri_total_cost)*lang_factor
        
        ##Doesn't include costs of indirect emissons
        total_emission_cost=((EAF_co2+cao_emission+co2_eaf_electrode+pellet_production)*emission_cost)
        ...
        operational_cost_annual=((iron_ore_cost_tls+electricity_cost_tls+eaf_op_cost_tls+dri_op_cost_tls)*steel_prod_yr)/10**6

        print('Electricity required in the Electric Arc Furnace in (kwh): ',np.around(el_eaf,2))
        print('Electricity required in the Electrolyzer in (kwh): ',np.around(el_elec,2))
        print('Electricity required in the Heater in (kwh): ',np.around(el_heater,2))
        ...
        #print('Total Capitol Cost Over Plant Life: ',np.sum(capitol_cost_yr))
        #print('Total Operational Cost Over Plant Life: ',np.sum(operational_cost))
        #print('Total Maintenance Cost Over Plant Life: ',np.sum(maintenance_cost))
        #print('Total Labor Cost Over Plant Life: ',np.sum(labour_cost))
        #print('Total Emission Cost Over Plant Life: ',np.sum(emission_cost))
        #print('Total Depreciation Cost Over Plant Life: ',np.sum(depreciation_cost))
        #print('Total Revenue Over Plant Life: ',np.sum(total_revenue))
    
        total_cost=total_capital_cost+(operational_cost_annual*plant_life)+(maintenance_cost_yr*plant_life)+(labour_cost_yr*plant_life)+(total_emission_cost*plant_life)+(depreciation_yr*plant_life)
        income=np.sum(total_revenue)-total_cost
