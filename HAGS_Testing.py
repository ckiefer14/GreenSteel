...
def establish_save_output_dict():
    ...
    save_outputs_dict['H2 Production per year kg'] = list()
    ...
    save_outputs_dict['Emissions tonne Co2'] = list()
    save_outputs_dict['Yearly Steel Production tls'] = list()
    save_outputs_dict['Cash Flow'] = list()
    save_outputs_dict['Levelized Cost of Hydrogen $/kg'] = list()
    save_outputs_dict['Levelized Cost of Oxygen $/kg'] = list()
    save_outputs_dict['Hydrogen from Electrolyzer to Storage kg'] = list()
...
def HDRI_EAF_Model(site_name,scenario_name,eta_el,h2_prod_yr,plant_life,tax_rate,interest_rate,electricity_cost,iron_ore_cost,emission_cost,carbon_steel_price,O2_price,el_spec,h2_investment_2020,emission_factor,elec_limit,perc_O2_sold):
    ...
    def electrolyzer_npv(eta_el,tax_rate,interest_rate,electricity_cost,iron_ore_cost,emission_cost,carbon_steel_price,O2_price,el_spec,h2_investment_2020,emission_factor):
        ...
        save_outputs_dict['Emissions tonne Co2'].append(total_emission)
        save_outputs_dict['Yearly Steel Production tls'].append(steel_prod_yr)
        save_outputs_dict['Cash Flow'].append(cash_flow)
        save_outputs_dict['Levelized Cost of Hydrogen $/kg'].append(lcoh)
        save_outputs_dict['Levelized Cost of Steel $/tls'].append(lcos_annual)
        save_outputs_dict['Levelized Cost of Oxygen $/kg'].append(lcoo)
